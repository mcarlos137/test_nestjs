...
type WsMessageParser = (data: WsData) => { method: string; operation: string, channel: string } | void;
...
export class WsAdapter extends AbstractWsAdapter {
    ...
    public bindMessageHandlers(
        ...
        handlers.forEach(handler => {
            handlersMap.set(handler.message, handler)
        });
    ...
    public bindMessageHandler(
        ...
        try {
            ...
            const messageHandler = handlersMap.get(message.method)!;
            const { callback } = messageHandler;
            return transform(callback({method: message.method, operation: message.operation, channel: message.channel}, message.method));
        } catch {
            return EMPTY;
        }
    }
